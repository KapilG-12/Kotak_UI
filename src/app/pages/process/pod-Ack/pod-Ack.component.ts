...
export class PodAckComponent implements OnInit {
  ...
  searchTableFileData($event) {
    let val = $event.target.value;
    if (val == '') {
      this.formattedFileData = this.immutableFormattedDataFile;
    } else {
      let filteredArrFile = [];
      const strArr = val.split(',');
      this.formattedFileData = this.immutableFormattedDataFile.filter(function (d) {
        for (var key in d) {
          strArr.forEach(el => {
            if (d[key] && el !== '' && (d[key] + '').toLowerCase().indexOf(el.toLowerCase()) !== -1) {
              if (filteredArrFile.filter(el => el.srNo === d.srNo).length === 0) {
                filteredArrFile.push(d);
              }
            }
          });
        }
      });
      this.formattedFileData = filteredArrFile;
    }
  }
  ...
  BindFileDetails(tableData, headerList) {
    ...
    this.immutableFormattedDataFile = JSON.parse(JSON.stringify(formattedFileData));
